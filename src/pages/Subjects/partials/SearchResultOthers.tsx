...
const SearchResultOthers = forwardRef<SearchResultRefOthers, SearchResultProps>(({ search, subject, sh }, ref) => {
    ...
    const { data = [], isFetching, error, refetch } = useQuery({
        queryKey: ["subjectFetchSearchOthers", search, page, subject, sh],
        queryFn: async () => {
            const res = await axios.get(
                `${config.baseUri}/api/subject/search-others?key=${search}&subj=${subject}&sh=${sh}&page=${page}`
            )
            return res.data
        },
    ...
    if (error) {
        return <div className="rounded-xl bg-red-50 p-4 text-sm text-red-700">Failed to load additional results.</div>
    }
